function [IFD, DFD, meansn, meanstn, meanca, cgIFD, cgDFD, cgmsn, cgmstn, cgmca] = SimpleCommSimulation_v2(par)
%This function is almost the same as SimpleCommSimulation() but slight modifications were made to accommodate generation of agents usingthe 2nd
%method.
%
...
for ai = 1:length(mrs)
    ...
    parfor gi = 1:length(gdivvals) %This is for parallel computing
        ...
        %Generate agents
        agents = GenAgent(par.numfuncs, par.numagents, [], exp((-(0:par.numfuncs-1).^2)/gdiv), par.anorm, 'Method', 2, 'MixingRatio', mr); %#ok<PFBNS> 
